...
export function buildReleaseApprovedTemplate(opts: {
  ...
  return emailLayout({
    ...
    preheader: `Ваш релиз «${opts.releaseTitle}» одобрен!`,
    body: `
      <div style="text-align:center;margin-bottom:20px;">
        <span style="font-size:48px;">✅</span>
      </div>
      ${heading('Релиз одобрен!')}
      ${bodyText(`<strong style="color:${T.textPrimary};">${opts.artistName}</strong>, ваш релиз <strong style="color:${T.accent};">«${opts.releaseTitle}»</strong> прошёл проверку и одобрен командой AURIX.`)}
