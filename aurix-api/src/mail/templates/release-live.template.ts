...
export function buildReleaseLiveTemplate(opts: {
  ...
  return emailLayout({
    ...
    preheader: `«${esc(opts.releaseTitle)}» теперь на площадках!`,
    body: `
      <div style="text-align:center;margin-bottom:20px;">
        <span style="font-size:48px;">🚀</span>
      </div>
      ${heading('Ваш релиз в эфире!')}
      ${bodyText(`<strong style="color:${T.textPrimary};">${esc(opts.artistName)}</strong>, ваш релиз <strong style="color:${T.accent};">«${esc(opts.releaseTitle)}»</strong> теперь доступен на музыкальных площадках!`)}
